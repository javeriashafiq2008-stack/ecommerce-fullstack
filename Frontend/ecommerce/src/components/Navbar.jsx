...
export default function Navbar() {
  ...
  const totalAmount = cart.reduce(
    (total, item) => total + Number(item?.price ?? item?.Product?.price ?? 0) * Number(item?.qty ?? item?.quantity ?? 1),
    0
  );
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <button
                ...
                className="text-white/90 hover:text-white transition-all duration-300 hover:scale-110 p-1 -ml-1 flex items-center justify-center"
              >
                <Menu className="w-6 h-6" />
              </button>

              <Link to="/" className="text-xl sm:text-2xl font-semibold text-white tracking-wide cursor-pointer">
                Jaydor
              </Link>
            </div>
            ...
            <div className="flex items-center space-x-3 sm:space-x-5 relative flex-shrink-0">

              {/* CART */}
              <button
                onClick={() => setIsCartOpen(!isCartOpen)}
                aria-label={`Open cart, ${cart.reduce((sum, item) => sum + Number(item?.qty ?? item?.quantity ?? 1), 0)} items`}
                className="relative flex items-center justify-center cursor-pointer group p-2 rounded-full hover:bg-white/10 transition-colors touch-manipulation focus:outline-none"
              >
                <svg
                  className="w-6 h-6 sm:w-5 sm:h-5 text-white/90 group-hover:text-white transition-all duration-300 group-hover:scale-110 group-hover:-rotate-6"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="1.5"
                  viewBox="0 0 24 24"
                ...
                <span className="pointer-events-none absolute -top-1 -right-1 bg-white text-black text-xs rounded-full px-1.5 font-bold leading-tight min-w-[18px] text-center shadow-sm">
                  {cart.reduce((sum, item) => sum + Number(item?.qty ?? item?.quantity ?? 1), 0) || 0}
                </span>
              </button>
      ...
      {shouldRenderCart && (
        <div className="fixed inset-0 z-50 overflow-hidden font-[Poppins]">
          ...
          <div className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-0 sm:pl-10">
            <div
              className={`pointer-events-auto w-screen max-w-full sm:max-w-md transform bg-[#f5f0ea] shadow-xl flex flex-col transition-transform duration-300 ease-in-out ${
                      ...
                      return (
                        <div key={item.id ?? item.name ?? index} className="relative flex gap-4 p-4 items-start">
                          ...
                          <div className="flex-1 min-w-0 pr-6">
                            <h4 className="text-sm font-semibold text-gray-900 leading-snug line-clamp-2">{itemName}</h4>
                            <p className="text-xs text-[#1a4d3c] mt-0.5">Jaydor</p>
                            <p className="text-sm font-semibold text-gray-900 mt-1.5">${itemPrice.toFixed(2)}</p>
                            ...
                            <div className="mt-2.5 inline-flex items-center bg-[#f5f0ea] rounded-full px-1 py-1">
                              ...
                              <span className="w-7 text-center text-sm font-medium text-gray-800">{itemQty}</span>
                              <button
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-3 py-5 space-y-6">
                ...
                <div className="space-y-1">
                  <p className="px-3 mb-2 text-[10px] font-bold uppercase tracking-widest text-white/35">Navigation</p>
                  <SidebarLink to="/" icon={ShoppingBag} label="Home" onNavigate={closeSidebar} />
                  <SidebarLink to="/products" icon={ShoppingBag} label="Products" onNavigate={closeSidebar} />
                  <SidebarLink to="/about" icon={User} label="About" onNavigate={closeSidebar} />
                  <SidebarLink to="/contact" icon={ClipboardList} label="Contact" onNavigate={closeSidebar} />
                  <button
                    onClick={() => {
                      closeSidebar();
                      setIsCartOpen(true);
                    }}
                    className="group w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-white/70 hover:text-white hover:bg-white/5 transition-all duration-200 hover:translate-x-0.5 text-left"
                  >
                    <ShoppingBag className="w-[18px] h-[18px] flex-shrink-0" />
                    <span className="flex-1 truncate">Cart</span>
